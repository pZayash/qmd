# collection-set-path Specification

## Purpose

Remap a collection's filesystem root after copying an existing SQLite index
(YAML + `store_collections.path`) without reindexing or re-embedding.
