# index-ls Specification

## Purpose

MCP tool `ls` browses one indexed directory level (files plus child folder
names from path prefixes) or lists collections when path is omitted. Inventory,
not search and not agent drill. Cap 200 children. No resource `list()` on
`qmd://`. CLI `qmd ls` stays a recursive dump.
