# mcp-http-bind Specification

## Purpose

HTTP MCP listen address is configurable (`--host` / `QMD_HOST`) while default
stays loopback. Non-loopback bind warns because HTTP MCP has no authentication.
