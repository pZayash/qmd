...
the same collection. Resolution domain is within-collection only. Anchors are not
used for resolution (`dst` resolves at document level). Wikilink resolution
SHALL match by document title or filename basename using the tie-break: exact
name match first, then shortest collection-relative path, otherwise dangling.
Relative markdown links SHALL resolve against the path relative to the source
document. The resolve pass SHALL rebuild all doc edges for the collection on
every reindex (no hash caching of resolved edges).
...
- **THEN** a doc edge `src → that document` of kind `wikilink` is created with a non-null `dst_doc_id`
